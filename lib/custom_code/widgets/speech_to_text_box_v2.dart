...
class SpeechToTextBoxV2 extends StatefulWidget {
  const SpeechToTextBoxV2({
  ...
  @override
  State<SpeechToTextBoxV2> createState() => _SpeechToTextBoxState();
}

class _SpeechToTextBoxState extends State<SpeechToTextBoxV2> {
  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => _ViewModel(
        remoteApi: _RemoteWhisperApi(apiKey: widget.apiKey),
...
class _ViewModel extends ChangeNotifier {
  ...
  final _RemoteWhisperApi remoteApi;
  ...
  void _whenRecordStopped() async {
    ...
    notifyListeners();
    try {
      final text = await remoteApi.extractText(_fileDir!);
      await File(_fileDir!).delete();

      FFAppState().update(() {
        FFAppState().whisperResult = text;
      });
...
//test
class _RemoteWhisperApi {
  _RemoteWhisperApi({required this.apiKey});
  ...
  Future<String> extractText(String filePath) async {
    ...
    return responseMap['text'] as String;
  }
}
...
class _FileManager {
  Future<String> createTempFile() async {
    ...
    final result = p.join(
      tempDir.path,
      'grow_up_audio_${DateTime.now().millisecondsSinceEpoch}.m4a',
    );
    return result;
  }
}
// Set your widget name, define your parameter, and then add thed
